{
	...
	'1.1 Game Layout': [
		...
		'types and two Mirror types, information on which is in 1.4.3. See 1.1.1 for a game summary table.'
	],
	'1.1.1 Game Summary': [
		'Score Items: 3',
	...
	'1.2 Satellite': [
		'Each team will write the software to command a SPHERES satellite to move in order to complete the game tasks.',
		'A SPHERE satellite can move in all directions using its twelve thrusters.',
		'The actual SPHERES satellites, like any other spacecraft, has a fuel source (in this case, liquid carbon dioxide) and a power source (in this case, AA battery packs).',
		'These resources are limited and must be used wisely.',
		'Therefore, the players of Zero Robotics are limited in the use of real fuel and batteries by virtual limits within the game.',
		'This section describes the limits to which players must adhere to in order to wisely use virtual SPHERES resources.'
	],
	'1.2.1 ZR User API': ['The non-game specific functions used to control the SPHERES satellite in Zero Robotics can be found in a document titled “ZR User API” on the Tutorials webpage under “Other Resources”. Game specific functions, along with a link to the standard ZR User API functions, are also provided in Section 4 of this document.'],
	'1.2.2 Time': ['Players have 180s gain as many points as possible. After 180s scores will be final and compared.'],
	'1.2.3 Fuel': [
		'Each player is assigned a virtual fuel allocation of 60 seconds, which is the total sum of fuel used in seconds of individual thruster firing.',
		'Once the allocation is consumed, the satellite will not be able to respond to SPHERES control commands.',
		'It will fire thrusters only to avoid leaving the Interaction Zone or colliding with the other satellite.',
		'Any action that requires firing the thrusters such as rotating or moving consumes fuel.',
		'Being out of bounds consumes an additional penalty of 2% of initial fuel per second.',
	...
	'1.2.4 Code Size': [
		...
		'When you compile your project with a code size estimate, the compiler will provide the percentage of the code size allocation that your project is using.',
		'Formal competition submissions require that your code size be 100% or less of the total allocation.'
	],
	'1.2.5 Noise': [
		'It is important to note that although the two competitors in a match will always be performing the same challenge and have identical satellites, the two satellites may be affected by random perturbations in different ways, resulting in small or even large variations in score.',
		'This is fully intended as part of the challenge and reflects uncertainties in the satellite dynamic and sensing models.',
		'The best performing solutions will be those that prove to be robust to these variations and a wide variety of object parameters.'
	],
	'1.3 Initial Position': [
		...
		'    Y: -0.6',
		'    Z: 0.0',
		'The satellite radius is 0.11m, but satellite position relative to game features is determined by the location of the centre of the satellite.',
	],
	'1.3.1 Player ID': [
	...
	'1.4.1 Energy': [
		'Energy is the most prohibitive resource the satellites utilize.',
		'Both players start with 5 energy at the start of the game, which is also the maximum energy a satellite can have.',
		'If the satellite is in the light zone, it gains 0.5 energy every second.',
		'To manoeuvre the satellite, 0.15 energy is used per 1 second of fuel used.',
		'Other activities that cost energy include taking and uploading pictures.',
	...
	'1.4.2 Light and Dark Zones': [
		'The Light and Dark Zones, and the Grey Zone in between them, will be the main determinant of how well you can take pictures of the other satellite.',
		'    Light Zone: Photos can be taken of you, you recharge energy.',
		'    Grey Zone: Photos can be taken of you, you do not recharge energy.',
		'    Dark Zone: Photos can not be taken of you, you do not recharge energy.',
		'The negative Y sector starts out in the Dark Zone and the positive section starts out in the Light Zone.',
		'In between them, with a total width of 0.2 meters centred at the origin, is the Grey Zone.',
		'Between 30 and 60 seconds into the game, the Light Zone and the Dark Zone will switch position.',
		'The zones will switch again periodically every 60 seconds until the end of the game.',
	...
	'1.4.3 Items': [
		...
		'There are nine total items (0-8), with all three item types.',
		'Each of them may only be used once, and they do not regenerate.',
		'Item types:',
		...
		'In order to collect an item, the centre of the user’s satellite must be no greater than 0.05 meters away from the item and it must be moving at slower than .01 meters per second.',
	],
	'1.4.3.1 Item Locations': [
	...
	'1.4.4 Picture Taking': [
		...
		'The function float takePic() will then take a picture if all of the conditions are met, and store it in the first available memory slot.',
		'Regardless of whether the picture was successfully taken, takePic() will consume 1 energy and shut down the camera for 3 seconds when it is called.',
		'The function will return 0 if picture-taking was unsuccessful, and the point value of the picture otherwise.',
		'Unsuccessful pictures do not use a memory slot.',
		'Every picture, whether successful or not, adds 0.01 points to the user’s score as a tiebreaker.',
		'The amount of points each picture is worth is determined by the distance between the two satellites when the picture is taken.',
		'It follows this formula: `points = 2.0 + 0.1 / (distance - PHOTO_MIN_DISTANCE + 0.1)` where PHOTO_MIN_DISTANCE is 0.5.',
		'If the opposing satellite is using an active mirror, the amount of points the picture is worth will zero.',
		'The function float getPicPoints() is available to check the points a picture will be worth before taking it.',
		'Calling it costs 0.1 energy, but does not disable the camera.',
		'Note that this function can sense whether or not the opponent has deployed a mirror.',
	],
	'1.4.5 Uploading Pictures': [
		...
		'When calling the function, the camera will be disabled for 3 seconds, similarly to taking pictures.',
		'Afterwards, uploadPics() will return the number of points that were uploaded.',
		'This process costs 1.0 energy. You do not receive points for pictures taken that are not uploaded.',
	],
	'1.4.6 End of Game': [
		'The game ends after 180s. Whichever team has more points wins.',
		'In the unlikely case of a tie, the satellite that is closer to the origin wins.',
	],
	'1.5 Scoring Summary': [
		'Attempting to Take a Picture (Valid or Not): 0.01',
		'Taking a Valid Picture: 0.1',
		'Uploading Pictures: 2.0 + 0.1/(distance – 0.5 + 0.1)',
	...
	'2.1. Date Notes': [
		'The leaderboard will be run on the dates listed above.',
		'Submissions close at 5pm AEST.',
		'Teams are not expected to have full working versions of their code ready for the initial leaderboard run.',
		'The teams that have begun writing and testing their initial strategy will be rewarded with additional tournament points from the early leaderboard runs.',
		'Perhaps the final code produced does not win the final leaderboard run however a consistent effort and accumulation of points could make this team the winner.',
		'The best strategy is to SUBMIT EARLY, SUBMIT OFTEN.',
	],
	...
	'3.2 Ethics Code': [
		...
		'Teams are strongly encouraged to report bugs as soon as they are found.',
		'Intentional abuse of an un-reported bug may be considered as unethical behaviour',
	...
	'4.1 Standard Zero Robotics API Reference': [
		'A guide to the standard Zero Robotics API functions is here: http://static.zerorobotics.mit.edu/docs/tutorials/ZR_user_API.pdf',
